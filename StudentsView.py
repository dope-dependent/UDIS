...
class StudentsView (StudentSearch.StudentSearch):
    ...
    def bindingAction(self, event):
        ...
        cursor_.execute('SELECT * FROM courses_taken WHERE roll=(:roll)',{'roll':roll})
    ...
    def fetchPerformance(self,roll_no):
        connect_,cursor_=ES.get_student_db_ES()
        cursor_.execute('SELECT * FROM courses_taken WHERE roll=(:roll)',{'roll':roll_no})
        allCourses=cursor_.fetchall()
        semDict={}
        for i in allCourses:
            if i[3] in semDict:
    ...
    def viewPerformance(self,roll_no,popupStudent,student):
        ...
        ind=0
        for i in semDict:
            self.semPerformance(perfView,2+ind,i,semDict[i])
            ind=ind+1
